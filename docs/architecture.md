...
| R3 | Exclusive-fullscreen games sit above the overlay and reset the gamma ramp | High | Detect via `SHQueryUserNotificationState` (`QUNS_RUNNING_D3D_FULL_SCREEN`) and suspend the overlay rather than fight z-order; re-apply on return to the desktop |
